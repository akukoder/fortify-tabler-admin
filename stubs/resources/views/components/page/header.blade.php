            ...
            <div class="nav-item dropdown">
                <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown" aria-label="Open user menu" aria-expanded="false">
                    <span
                        ...
                        style="background-image: url('@if (Auth::user()->avatar) {{ asset('storage/avatars/'.Auth::user()->avatar) }} @else {{ config('avatar.service_url') }}?name={{ urlencode(Auth::user()->name) }}&color=fff&background={{ substr(md5(Auth::user()->name), 0, 6) }}&size={{ config('avatar.size') }} @endif')">
                    </span>
